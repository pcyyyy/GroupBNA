...
def x1_x2(data,AD_feature,LMCI_feature,MCI_feature,EMCI_feature,NC_feature,edge_id,drop_rate):
    ...
    if param['drop_scheme'] == 'degree':
        ...
        if args.dataset == 'WikiCS':
            feature_weights = feature_drop_weights_dense(data.x, node_c=feature_martix).to(device)
        else:
            feature_weights = feature_drop_weights_dense(data.x, node_c=feature_martix).to(device)
    else:
        feature_weights = torch.ones((data.x.size(1),)).to(device)

    edge_index = drop_edge(edge_id, data)
    x = drop_feature(data.x, drop_rate)

    if param['drop_scheme'] in ['pr', 'degree', 'evc']:
        x = drop_feature_weighted_2(data.x, feature_weights, drop_rate)
...
def train(dataset,device,AD_feature,LMCI_feature,MCI_feature,EMCI_feature,NC_feature):
    ...
    loader1 = DataLoader(dataset, batch_size=64, shuffle=False)
    total_loss = 0
    ...
    for data in loader1:
        ...
        for i in range(data.batch_size):
            x1, edge_index1 = x1_x2(data[i], AD_feature, LMCI_feature, MCI_feature, EMCI_feature, NC_feature, 1,
                                    drop_rate=param['drop_feature_rate_1'])
            x2, edge_index2 = x1_x2(data[i], AD_feature, LMCI_feature, MCI_feature, EMCI_feature, NC_feature, 2,
                                    drop_rate=param['drop_feature_rate_2'])
            drop_data1 = Data(x=x1, edge_index=edge_index1)
            drop_data2 = Data(x=x2, edge_index=edge_index2)
            drop_list1.append(drop_data1)
            drop_list2.append(drop_data2)

        batch1 = Batch.from_data_list(drop_list1)
        batch2 = Batch.from_data_list(drop_list2)

        z1 = model(batch1.x, batch1.edge_index)
        z2 = model(batch2.x, batch2.edge_index)
